...
public class OrderMarshaller implements DataRowSetMarshaller<Order> {
    ...
    private static MetaField fillDate = MetaFieldFactory
            .createDate("order.fillDate");
    private static MetaField priceType = MetaFieldFactory
            .createText("order.priceType");
    private static MetaField status = MetaFieldFactory
            .createText("order.status");
    private static MetaField price = MetaFieldFactory
            .createDecimal("order.price");
    private static MetaField quantity = MetaFieldFactory
            .createDecimal("order.quantity");
    private static MetaField fillPrice = MetaFieldFactory
            .createDecimal("order.fillPrice");
    private static MetaField fillQuantity = MetaFieldFactory
            .createDecimal("forder.fillQuantity");
    private static MetaField orderLegs = MetaFieldFactory
            .createRowset("order.orderLegs");
    private static Metadata responseMeta = Metadata.from(orderId, SharedMeta.accountId,
            date, fillDate, SharedMeta.marketSession, SharedMeta.symbol, SharedMeta.exchange, status, priceType,
    ...
    public DataRowSet marshal(Order order) {
        ...
        rowset.addValueAtRow(SharedMeta.accountId, order.getAccount().getId(), 0);
        rowset.addValueAtRow(SharedMeta.marketSession, order.getMarketSession().name(), 0);
        rowset.addValueAtRow(SharedMeta.symbol, order.getSecurity().getSymbol(), 0);
